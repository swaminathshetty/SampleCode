//
//  TDGlucoseDataContentProvider.m
//  AllHealthChoice
//
...
@implementation TDGlucoseDataContentProvider
...
- (void) addGlucoseData: (TDGlucoseData *) data
{
    ...
    if(sqlite3_open([databasePath UTF8String], &dbhandle) == SQLITE_OK)
    {
        ...
        sql = @"INSERT INTO MData(MType, Value1, Value2, Value3, Value4, Value5, RawData, MDateTime, RecStatus, MeterId, userNO) VALUES(1, ?, 0, ?, ?, 0, '', ?, ?, ?, ?)";
        ...
        if(sqlite3_prepare_v2(dbhandle, [sql UTF8String], -1, &compiledStatement, NULL) == SQLITE_OK)
        {
            ...
            sqlite3_bind_int( compiledStatement, 2, data.type);
            sqlite3_bind_int( compiledStatement, 3, data.machineOrgType);
            sqlite3_bind_int( compiledStatement, 4, data.inputDate);
            sqlite3_bind_int( compiledStatement, 5, data.upload);
            sqlite3_bind_int( compiledStatement, 6, data.meterUID);
            
            sqlite3_bind_int( compiledStatement, 7, data.userNO);
...
- (int) checkDataRepeat: (int)MDateTime Data1:(double)value1
{
    ...
    sql = [NSString stringWithFormat:@"SELECT COUNT(_id) FROM MData WHERE MType = 1 MDateTime = ? AND Value1 = ?"];
